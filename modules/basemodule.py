...
class BaseModule(ABC):
    ...
    def read(self, controller: Controller, source: str, code: str, send_to: str, line: list):
        """
        ...
        """
        self.process(controller, source, code, send_to, line)
        self.delegate(controller, source, code, send_to, line)
    ...
    def delegate(self, controller: Controller, source: str, code: str, send_to: str, line: list):
        """
        ...
        if len(line) >= 4 and len(line[3]) > 1:
            if line[3][1] == util.command_prefix:
                ...
                if callable(cmd_func) and hasattr(cmd_func, "is_command"):
                    cmd_func(controller, source, code, send_to, line)
